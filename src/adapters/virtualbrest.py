...
def _parse_list(html: str) -> List[str]:
    ...
    for a in soup.select("a[href]"):
        ...
        if "afisha" in full or "event" in full or "anons" in full:
            links.append(full)
    seen = set()
    uniq: List[str] = []
    for u in links:
...
def harvest_virtualbrest(client: HttpClient, geocoder: Geocoder, limit: int = 50) -> List[Event]:
    results: List[Event] = []
